...
CogAgent::~CogAgent()
{
    if (_enabled)
    {
        Util::AxisAlignedBox bounds(_position.x - _radius, _position.x + _radius, 0.0f, 0.0f, _position.z - _radius,
                                    _position.z + _radius);
...
{
    // compute the "old" bounding box of the agent before it is reset.  its OK that it will be invalid if the agent was
    // previously disabled because the value is not used in that case.
    ...
    Util::AxisAlignedBox newBounds(_position.x - _radius, _position.x + _radius, 0.0f, 0.0f, _position.z - _radius,
                                   _position.z + _radius);
...
SteerLib::EngineInterface *CogAgent::getSimulationEngine() { return gEngine; }
...
bool CogAgent::prepareGoalQuery()
{
    ...
    // for the simple AI, if the agent"s distance to its goal is less than its radius, then the agent has reached the
    // goal.
...
bool CogAgent::prepareCollisionQuery()
{
    ...
    getSimulationEngine()->getSpatialDatabase()->getItemsInRange(
        _neighbors, _position.x - (this->_radius + COL_QUERY_RADIUS), _position.x + (this->_radius + COL_QUERY_RADIUS),
        _position.z - (this->_radius + COL_QUERY_RADIUS), _position.z + (this->_radius + COL_QUERY_RADIUS), (this));
    ...
    for (auto &it : _neighbors)
    {
        ...
        if (agent != NULL)
        {
            if (agent->overlaps(_position, _radius))
            {
        ...
        if (obstacle != NULL)
        {
            if (obstacle->overlaps(_position, _radius))
            {
